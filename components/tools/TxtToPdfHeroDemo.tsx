...
/**
 * TxtToPdf hero demo — Mac-style mockup showing a .txt file
 * being converted to a PDF client-side.
 *
 * Phases:
 *   "idle"       — .txt card visible, "Convert to PDF" button pulses
 *   "processing" — spinner animates
 *   "done"       — PDF card pops in with page-lines preview
 *
...
const SOURCE = { filename: "notes.txt", chars: "4.2K chars" };
const OUTPUT = { filename: "notes.pdf", pages: 3 };

const IDLE_MS = 1600;
const PROCESSING_MS = 900;
const DONE_MS = 2400;

export default function TxtToPdfHeroDemo() {
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <div className="flex items-center gap-2 px-2 py-1.5 border-b border-[#E5E5E5] dark:border-[#2A2A2A] bg-[#F5F5F5] dark:bg-[#1E1E1E]">
          ...
          <span className="text-[10px] font-medium text-[#737373] ml-1">
            TXT to PDF &middot; {SOURCE.filename}
          </span>
          ...
          {!isDone ? (
            <>
                ...
                >
                  ...
                  <div className="relative w-8 h-10 rounded-sm bg-[#EF4444] flex items-center justify-center flex-shrink-0 shadow-sm">
                    <div className="flex flex-col gap-[3px] px-1">
                      <div className="h-[2px] bg-white/90 rounded-full w-5" />
                      <div className="h-[2px] bg-white/70 rounded-full w-4" />
                      <div className="h-[2px] bg-white/90 rounded-full w-5" />
                      <div className="h-[2px] bg-white/60 rounded-full w-3" />
                    </div>
                    <div className="absolute -bottom-1.5 -right-1.5 w-4 h-4 rounded-full bg-[#737373] border border-white dark:border-[#171717] flex items-center justify-center">
                      <span className="text-[6px] font-black text-white leading-none">TXT</span>
                    </div>
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-[#737373] tabular-nums">
                      <span className="font-semibold text-[#EF4444]">{SOURCE.chars}</span>
                    </div>
              ...
              {isProcessing && (
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span className="text-[10px] text-[#737373]">Converting to PDF&hellip;</span>
                </div>
              )}
              ...
              {!isProcessing && (
                <button
                  ...
                  style={{
                    animation: phase === "idle" ? "txtp-pulse 1.4s ease-in-out infinite" : "none",
                  }}
                  tabIndex={-1}
                  aria-hidden="true"
                >
            ...
            <div className="flex-1 flex flex-col justify-center gap-2">
              ...
              <div
                ...
                style={{
                  animation: "txtp-pop 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards",
                  opacity: 0,
                }}
              >
                PDF created
              ...
              <div
                ...
                style={{
                  animation: "txtp-pop 0.45s cubic-bezier(0.34,1.56,0.64,1) 0.1s forwards",
                  opacity: 0,
                }}
              >
                {[1, 2, 3].map((n) => (
                  <div key={n} className="relative flex flex-col items-center gap-0.5">
                    <div
                      className="w-12 rounded-sm bg-white dark:bg-[#1E1E1E] border border-[#E5E5E5] dark:border-[#333] shadow-sm flex flex-col justify-between p-1.5"
                      style={{ height: "60px" }}
                    >
                      {/* Text lines */}
                      <div className="space-y-1">
                        <div className="h-[2px] bg-[#D4D4D4] dark:bg-[#333] rounded-full w-full" />
                        <div className="h-[2px] bg-[#D4D4D4] dark:bg-[#333] rounded-full w-4/5" />
                        <div className="h-[2px] bg-[#D4D4D4] dark:bg-[#333] rounded-full w-full" />
                        <div className="h-[2px] bg-[#D4D4D4] dark:bg-[#333] rounded-full w-3/4" />
                        <div className="h-[2px] bg-[#D4D4D4] dark:bg-[#333] rounded-full w-full" />
                      </div>
                      {/* Page badge */}
                      <div className="flex justify-end">
                        <span className="text-[8px] font-black text-[#EF4444] tabular-nums">{n}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div
                ...
                style={{
                  animation: "txtp-pop 0.45s cubic-bezier(0.34,1.56,0.64,1) 0.2s forwards",
                  opacity: 0,
                }}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-[9px] text-[#16A34A] font-semibold">
                    {OUTPUT.pages} pages &middot; no upload
                  </span>
                </div>
                <button
      ...
      <div className="hidden md:flex items-center justify-center gap-1.5 mt-1.5 text-[10px] text-[#737373] dark:text-[#A3A3A3]">
        ...
        <span>Converts .txt to PDF &middot; word-wrap &middot; 100% in your browser &middot; no upload</span>
      </div>

      <style jsx>{`
        @keyframes txtp-pulse {
