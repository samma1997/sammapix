...
const ACCEPT = ".apk,.xapk,.zip,application/zip,application/x-zip-compressed,application/vnd.android.package-archive";
...
/** Return a friendly label for the APK variant */
function apkTypeLabel(filename: string): string {
  const ext = filename.split(".").pop()?.toLowerCase();
  if (ext === "xapk") return ".xapk (Split APK Bundle)";
  if (ext === "apk") return ".apk (Android App Package)";
  return ".zip";
}

/** True if the file is an APK-like archive we can open */
function isApkLike(file: File): boolean {
...
export default function ApkExtractorClient() {
  ...
  const parseFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setEntries(parsed);
      setUiState("results");
      trackEvent("apk_extractor_opened", {
        ext: file.name.split(".").pop() ?? "",
        files: parsed.filter((e) => !e.isDir).length,
      });
    } catch {
      setErrorMsg(
        "This file could not be opened as an APK/ZIP archive. Make sure it is a valid .apk, .xapk or .zip and is not corrupted."
      );
      setUiState("error");
    }
  }, []);
  ...
  const handleFile = useCallback(
    (file: File) => {
      if (!isApkLike(file)) {
        setErrorMsg(
          `"${file.name}" is not a supported file type. Accepted: .apk, .xapk, .zip`
        );
        setSourceFile(null);
        setUiState("error");
        return;
  ...
  const downloadSingle = useCallback(
    (entry: PackEntry) => {
      ...
      trackEvent("apk_extractor_download_single", { name: entry.name });
  ...
  const downloadAllZip = useCallback(async () => {
    ...
    if (!isPro) {
      trackEvent("apk_extractor_zip_gate", { files: files.length });
    ...
    try {
      ...
      const name = sourceFile
        ? sourceFile.name.replace(/\.(apk|xapk)$/i, "-extracted.zip")
        : "apk-extracted.zip";
      saveAs(blob, name);
      trackEvent("apk_extractor_download_zip", { files: files.length });
      handleDownloadSuccess();
    } catch {
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-4 pb-16">
      ...
      {uiState === "idle" && (
        <div
          ...
          role="button"
          aria-label="Drop an APK file or click to select"
        >
          <input
            ref={fileInputRef}
            type="file"
          ...
          <p className="text-sm text-[#737373] dark:text-[#A3A3A3] mb-4">
            .apk, .xapk or .zip — click to select
          </p>
          ...
          <p className="text-xs text-[#A3A3A3] dark:text-[#525252] mt-1">
            No Android device or tools to install.
          </p>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-2 px-3 py-2.5 bg-[#FFF7ED] dark:bg-[#1C0F00]/40 border border-[#FED7AA] dark:border-[#92400E]/40 rounded-lg">
            ...
            <p className="text-xs text-amber-800 dark:text-amber-300">
              This tool extracts the APK&apos;s raw files. It does not decompile bytecode (classes.dex) or decode AndroidManifest.xml — those require tools like apktool.
            </p>
          </div>
          ...
          <p className="text-xs text-[#A3A3A3] dark:text-[#525252] text-center">
            Folder structure is preserved exactly as inside the APK.
          </p>
        </div>
      )}
      ...
      {(uiState === "idle" || uiState === "error") && (
        <div className="mt-6 flex flex-wrap gap-3 justify-center text-xs text-[#A3A3A3] dark:text-[#525252]">
          ...
          <span className="flex items-center gap-1">
            Need to open a Minecraft pack?{" "}
            <Link
              href="/tools/minecraft-extractor"
              className="text-[#0EA5E9] hover:underline inline-flex items-center gap-0.5"
            >
              Minecraft Extractor <ChevronRight size={10} />
            </Link>
          </span>
        </div>
      )}

      <FreeSignupAdBar tool="apk-extractor" />
