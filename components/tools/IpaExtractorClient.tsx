...
const ACCEPT = ".ipa,application/octet-stream,application/zip,application/x-zip-compressed";
...
/** True if the file is an IPA archive we can open */
function isIpaLike(file: File): boolean {
  return file.name.toLowerCase().endsWith(".ipa");
}

/** Try to extract app name and bundle id from Info.plist if it is plain-text XML.
 *  Returns null if the plist is binary (bplist) or parsing fails. */
function parseInfoPlist(content: string): { appName: string; bundleId: string } | null {
  try {
    // Binary plists start with "bplist" — bail out immediately
    if (content.startsWith("bplist")) return null;
    const getName = (key: string): string | null => {
      const re = new RegExp(`<key>${key}</key>\\s*<string>([^<]+)</string>`, "i");
      const m = content.match(re);
      return m ? m[1] : null;
    };
    const appName = getName("CFBundleDisplayName") ?? getName("CFBundleName");
    const bundleId = getName("CFBundleIdentifier");
    if (!appName && !bundleId) return null;
    return { appName: appName ?? "", bundleId: bundleId ?? "" };
  } catch {
    return null;
  }
}
...
export default function IpaExtractorClient() {
  ...
  const [appInfo, setAppInfo] = useState<{ appName: string; bundleId: string } | null>(null);
  ...
  const parseFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      // Try to parse Info.plist if it is an XML plist (many are binary — skip those)
      const plistEntry = parsed.find(
        (e) =>
          !e.isDir &&
          /^Payload\/[^/]+\.app\/Info\.plist$/.test(e.name)
      );
      if (plistEntry?.blob) {
        try {
          const text = await plistEntry.blob.text();
          const info = parseInfoPlist(text);
      ...
      setUiState("results");
      trackEvent("ipa_extractor_opened", {
        files: parsed.filter((e) => !e.isDir).length,
      });
    } catch {
      setErrorMsg(
        "This file could not be opened as an IPA/ZIP archive. Make sure it is a valid .ipa and is not corrupted."
      );
      setUiState("error");
    }
  }, []);
  ...
  const handleFile = useCallback(
    (file: File) => {
      if (!isIpaLike(file)) {
        setErrorMsg(
          `"${file.name}" is not a supported file type. Accepted: .ipa`
        );
        setSourceFile(null);
        setUiState("error");
        return;
  ...
  const downloadSingle = useCallback(
    (entry: PackEntry) => {
      ...
      trackEvent("ipa_extractor_download_single", { name: entry.name });
  ...
  const downloadAllZip = useCallback(async () => {
    ...
    if (!isPro) {
      trackEvent("ipa_extractor_zip_gate", { files: files.length });
    ...
    try {
      ...
      const name = sourceFile
        ? sourceFile.name.replace(/\.ipa$/i, "-extracted.zip")
        : "ipa-extracted.zip";
      saveAs(blob, name);
      trackEvent("ipa_extractor_download_zip", { files: files.length });
      handleDownloadSuccess();
    } catch {
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-4 pb-16">
      ...
      {uiState === "idle" && (
        <div
          ...
          role="button"
          aria-label="Drop an IPA file or click to select"
        >
          <input
            ref={fileInputRef}
            type="file"
          ...
          <p className="text-xs text-[#A3A3A3] dark:text-[#525252] mt-1">
            No iOS device, Xcode or iTunes required.
          </p>
        </div>
      )}
      ...
      {uiState === "results" && sourceFile && (
        <div className="space-y-4">
          ...
          {/* App info banner (only when Info.plist is XML-readable) */}
          {appInfo && (appInfo.appName || appInfo.bundleId) && (
            <div className="flex items-center gap-2.5 px-4 py-3 bg-[#F0F9FF] dark:bg-[#0C1B2A]/60 border border-[#BAE6FD] dark:border-[#0369A1]/40 rounded-xl">
              <div className="w-8 h-8 rounded-lg bg-[#0EA5E9]/15 flex items-center justify-center shrink-0">
                <span className="text-[10px] font-bold text-[#0EA5E9]">APP</span>
              </div>
              <div className="flex-1 min-w-0">
                {appInfo.appName && (
                  <p className="text-sm font-semibold text-[#0C4A6E] dark:text-[#BAE6FD] truncate">
                    {appInfo.appName}
                  </p>
                )}
                {appInfo.bundleId && (
                  <p className="text-xs text-[#0369A1] dark:text-[#7DD3FC] font-mono truncate">
                    {appInfo.bundleId}
                  </p>
                )}
              </div>
          ...
          {/* Disclaimer: inspection only, no install */}
          <div className="flex items-start gap-2 px-3 py-2.5 bg-[#FFF7ED] dark:bg-[#1C0F00]/40 border border-[#FED7AA] dark:border-[#92400E]/40 rounded-lg">
            <AlertCircle size={13} className="text-amber-500 shrink-0 mt-0.5" strokeWidth={1.5} />
            <p className="text-xs text-amber-800 dark:text-amber-300">
              This tool extracts and inspects the IPA&apos;s raw files. It cannot install the app on a device — IPA files require a valid provisioning profile and a trusted device or Xcode to install.
            </p>
          </div>
          ...
          <p className="text-xs text-[#A3A3A3] dark:text-[#525252] text-center">
            Folder structure is preserved exactly as inside the IPA.
          </p>
        </div>
      )}
      ...
      <FreeSignupAdBar tool="ipa-extractor" />
