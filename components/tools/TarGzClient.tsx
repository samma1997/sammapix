...
const ACCEPT =
  ".tar.gz,.tgz,.tar,.gz,.tar.bz2,.tbz2,.tar.xz,.txz,application/gzip,application/x-tar,application/x-bzip2,application/x-xz";
const POLL_INTERVAL_MS = 3000;
const POLL_MAX_MS = 5 * 60 * 1000;
const VALID_EXT = /\.(tar\.gz|tgz|tar\.bz2|tbz2|tar\.xz|txz|tar|gz)$/i;
...
function totalBytes(entries: Entry[]): number {
...
export default function TarGzClient() {
  ...
  useEffect(() => () => { cancelExtraction(); stopPolling(); }, [cancelExtraction, stopPolling]);
  ...
  const startExtraction = useCallback(async (file: File) => {
    ...
    try {
      ...
      const initial: Entry[] = fileEntries.map(({ file: f, path }) => ({
        name: entryName(f, path), size: f.size, status: "pending",
      }));
      setEntries([...initial]);
      trackEvent("targz_extracting", { files: fileEntries.length });

      const extracted: Entry[] = [...initial];
      for (let i = 0; i < fileEntries.length; i++) {
        if (cancelRef.current) { await archive.close(); return; }
        const { file: libFile, path } = fileEntries[i];
        const name = entryName(libFile, path);
        try {
          const ef = await libFile.extract();
          const buf = await ef.arrayBuffer();
      ...
      setUiState("results");
      setProgress(100);
      trackEvent("targz_extracted", { files: fileEntries.length });
    } catch (err: unknown) {
  ...
  const startListOnly = useCallback(async (file: File) => {
    ...
    try {
      ...
      await archive.close();
      if (cancelRef.current) return;
      const list: Entry[] = filesArray
        .filter(({ file: f, path }) => {
          const n = entryName(f, path);
          return !(n.endsWith("/") && f.size === 0);
        })
        .map(({ file: f, path }) => ({ name: entryName(f, path), size: f.size, status: "pending" }));
      if (list.length === 0) {
        setErrorMsg("No files found in the archive.");
        setUiState("error");
        return;
      ...
      try { await archive?.close(); } catch { /* ignore */ }
      if (cancelRef.current) return;
      setErrorMsg(err instanceof Error ? err.message : "Could not read archive.");
      setUiState("error");
    }
  }, []);
  ...
  const handleFile = useCallback(
    (file: File) => {
      if (!VALID_EXT.test(file.name.toLowerCase())) {
        setErrorMsg("Please select a valid .tar.gz, .tgz, .tar or .gz file.");
        setUiState("error");
        return;
      }
      setArchiveFile(file);
      ...
      if (file.size > limitBytes && !isPro) {
        startListOnly(file);
        trackEvent("targz_gate_shown", { size_mb: Math.round(file.size / 1024 / 1024) });
        return;
      }
  ...
  const startPolling = useCallback((file: File, sessionId: string) => {
    ...
    pollTimerRef.current = setInterval(async () => {
      ...
      try {
        const res = await fetch(`/api/day-pass/checkout-status?session_id=${encodeURIComponent(sessionId)}`);
        if (!res.ok) return;
        const data = (await res.json()) as { paid: boolean; email?: string };
        if (data.paid) {
          stopPolling();
          if (data.email) setGuestEmail(data.email);
          trackEvent("targz_daypass_unlocked_poll");
          startExtraction(file);
        }
  ...
  const handleUnlockClick = useCallback(async () => {
    ...
    trackEvent("targz_daypass_checkout_start");
    try {
      const res = await fetch("/api/checkout/day-pass", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ source: "/tools/tar-gz" }),
      });
      if (res.status === 409) { startExtraction(archiveFile); return; }
      if (!res.ok) {
  ...
  const downloadFile = useCallback((entry: Entry) => {
    ...
    trackEvent("targz_download_single");
  }, []);
  ...
  const downloadAllAsZip = useCallback(async () => {
    ...
    try {
      ...
      const name = archiveFile ? archiveFile.name.replace(VALID_EXT, ".zip") : "archive.zip";
      saveAs(blob, name.endsWith(".zip") ? name : `${name}.zip`);
      trackEvent("targz_download_zip", { files: ready.length });
    } catch {
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-4 pb-16">
      {justUnlockedViaRedirect && (
        <div className="mb-4 flex items-start gap-3 px-4 py-3 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/50 rounded-xl">
          ...
          <div>
            <p className="text-sm font-medium text-emerald-800 dark:text-emerald-300">You&apos;re unlocked for 24 hours.</p>
            <p className="text-xs text-emerald-700 dark:text-emerald-400 mt-0.5">Drop your .tar.gz file again below to extract it instantly.</p>
          </div>
        </div>
      )}
      ...
      {uiState === "idle" && (
        <div
          ...
          role="button"
          aria-label="Drop a .tar.gz file or click to select"
        >
          ...
          <p className="text-base font-medium text-[#171717] dark:text-[#E5E5E5] mb-1">Drop a .tar.gz file here</p>
          <p className="text-sm text-[#737373] dark:text-[#A3A3A3] mb-4">or click to select from your computer</p>
          <span className="inline-flex items-center gap-1.5 text-xs text-[#A3A3A3] dark:text-[#525252]">
            <Lock size={11} /> Files never leave your device. 100% in-browser
          </span>
          <p className="text-xs text-[#A3A3A3] dark:text-[#525252] mt-1">.tar.gz · .tgz · .tar · .gz · .tar.bz2 · Free up to 200 MB</p>
        </div>
      )}
      ...
      {(uiState === "loading" || uiState === "listing" || uiState === "extracting") && (
        <div className="border border-[#E5E5E5] dark:border-[#2A2A2A] rounded-2xl p-8 text-center">
          ...
          <p className="text-sm font-medium text-[#171717] dark:text-[#E5E5E5] mb-2">
            {uiState === "loading" ? "Reading file..." : uiState === "listing" ? "Scanning archive..." : `Extracting files ${progress}%`}
          </p>
          {uiState === "extracting" && (
            <>
              <div className="w-full bg-[#F5F5F5] dark:bg-[#252525] rounded-full h-1.5 overflow-hidden mt-4">
                <div className="h-full bg-[#6366F1] transition-all duration-200 rounded-full" style={{ width: `${progress}%` }} />
              </div>
              {entries.length > 0 && (
                <p className="text-xs text-[#A3A3A3] dark:text-[#525252] mt-2">{readyEntries.length} / {entries.length} files extracted</p>
              )}
            </>
          )}
        </div>
      ...
      {uiState === "filelist_gate" && archiveFile && (
        <div className="space-y-4">
          ...
          <div className="border border-[#E5E5E5] dark:border-[#2A2A2A] rounded-xl overflow-hidden divide-y divide-[#F5F5F5] dark:divide-[#252525]">
            ...
            {entries.length > 8 && (
              <div className="px-4 py-2.5 bg-[#FAFAFA] dark:bg-[#191919] text-xs text-[#A3A3A3] dark:text-[#525252]">+ {entries.length - 8} more file{entries.length - 8 !== 1 ? "s" : ""}</div>
            )}
          </div>
          <div className="border border-[#6366F1]/30 dark:border-[#6366F1]/20 bg-[#F5F3FF] dark:bg-[#1E1B4B]/30 rounded-2xl p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-[#171717] dark:text-[#E5E5E5] mb-0.5">This archive is {fileSizeMB} MB, over the 200 MB free limit</p>
                <p className="text-xs text-[#737373] dark:text-[#A3A3A3] mb-4">Unlock extraction and download instantly. One pass, all tools, 24 hours.</p>
                <button onClick={handleUnlockClick} className="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold bg-[#6366F1] hover:bg-[#4F46E5] active:scale-[0.98] text-white rounded-xl transition-all shadow-sm">
                  <Zap size={15} strokeWidth={2} /> Unlock &amp; extract for $2.99
                </button>
                <p className="text-xs text-[#A3A3A3] dark:text-[#525252] mt-2">24-hour Day Pass · No subscription · Instant unlock</p>
              </div>
            </div>
          </div>
          <div className="text-center">
            <button onClick={handleReset} className="text-xs text-[#A3A3A3] dark:text-[#525252] hover:text-[#737373] dark:hover:text-[#A3A3A3] transition-colors">Choose a different file</button>
          </div>
        </div>
      )}
        ...
        <div className="border border-[#E5E5E5] dark:border-[#2A2A2A] rounded-2xl p-8 text-center space-y-4">
          {pollTimedOut ? (
            <>
              ...
              <p className="text-xs text-[#737373] dark:text-[#A3A3A3]">If you completed the payment, drop the file again. Your pass is active.</p>
              <div className="flex justify-center gap-3">
                <button onClick={() => { setPollTimedOut(false); handleReset(); }} className="px-4 py-2 text-sm font-medium border border-[#E5E5E5] dark:border-[#2A2A2A] text-[#171717] dark:text-[#E5E5E5] rounded-lg hover:bg-[#FAFAFA] dark:hover:bg-[#252525] transition-colors">Drop file again</button>
                <button onClick={handleUnlockClick} className="px-4 py-2 text-sm font-medium bg-[#6366F1] hover:bg-[#4F46E5] text-white rounded-lg transition-colors">Try again</button>
              </div>
            </>
          ) : (
            <>
              ...
              <p className="text-xs text-[#737373] dark:text-[#A3A3A3]">Complete the checkout in the popup. Your file will extract automatically the moment the payment goes through.</p>
              <div className="inline-flex items-center gap-1.5 text-xs text-[#A3A3A3] dark:text-[#525252]">
                <ExternalLink size={11} strokeWidth={1.5} /> Popup not showing?{" "}
                <button onClick={handleUnlockClick} className="text-[#6366F1] hover:underline">Open checkout again</button>
              </div>
            </>
          )}
        </div>
      ...
      {uiState === "error" && (
        <div className="border border-red-200 dark:border-red-900/40 bg-red-50 dark:bg-red-950/20 rounded-2xl p-6 flex flex-col items-center text-center gap-3">
          ...
          <p className="text-sm font-medium text-[#171717] dark:text-[#E5E5E5]">{errorMsg || "Extraction failed"}</p>
          <button onClick={handleReset} className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium bg-[#171717] dark:bg-white text-white dark:text-[#171717] rounded-lg hover:opacity-90 transition-opacity">
            <RotateCcw size={14} strokeWidth={1.5} /> Try another file
          </button>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-[#737373] dark:text-[#A3A3A3]">{readyEntries.length} file{readyEntries.length !== 1 ? "s" : ""}</p>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={handleReset} className="inline-flex items-center gap-1 px-3 py-1.5 text-xs text-[#737373] dark:text-[#A3A3A3] hover:text-[#171717] dark:hover:text-[#E5E5E5] border border-[#E5E5E5] dark:border-[#2A2A2A] rounded-lg transition-colors">
                <RotateCcw size={11} strokeWidth={1.5} /> New file
              </button>
            </div>
          </div>
          ...
          {guestEmail && !session?.user?.email && (
            <div className="flex items-start gap-2 px-3 py-2.5 bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800/40 rounded-xl">
              ...
              <p className="text-xs text-indigo-800 dark:text-indigo-300">
                ...
                <a href="/auth/signin?callbackUrl=/tools/tar-gz" className="font-medium underline underline-offset-2 hover:opacity-80 transition-opacity">Sign in with {guestEmail}</a>{" "}
          ...
          <div className="flex items-center gap-2 px-3 py-2 bg-[#F5F5F5] dark:bg-[#252525] rounded-lg">
            ...
            <p className="text-xs text-[#737373] dark:text-[#A3A3A3]">Files were extracted locally in your browser and are never uploaded.</p>
          </div>

          {/* One-click ZIP — convenient path at the moment of value. On tar-gz
              the ZIP is free for everyone (low-traffic tool, not worth gating),
              so this is a pure convenience shortcut with no upsell claim. */}
          {readyEntries.length > 1 && (
            <div className="flex items-center gap-3 px-4 py-3.5 bg-white dark:bg-[#191919] border border-[#E5E5E5] dark:border-[#2A2A2A] rounded-xl">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-[#171717] dark:text-[#E5E5E5]">Save all {readyEntries.length} files at once</p>
                <p className="text-xs text-[#737373] dark:text-[#A3A3A3]">Bundle everything into one .zip — no clicking file by file.</p>
              </div>
              <button onClick={downloadAllAsZip} disabled={zipBuilding} className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold bg-[#171717] dark:bg-white text-white dark:text-[#171717] rounded-lg hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-50 shrink-0">
                {zipBuilding ? <Loader2 size={13} strokeWidth={1.75} className="animate-spin" /> : <Download size={13} strokeWidth={1.75} />}
                Download .zip
              </button>
            </div>
          )}
          ...
          <div className="border border-[#E5E5E5] dark:border-[#2A2A2A] rounded-xl overflow-hidden divide-y divide-[#F5F5F5] dark:divide-[#252525]">
            {entries.map((entry) => (
              <div key={entry.name} className="flex items-center gap-3 px-4 py-3 bg-white dark:bg-[#191919] hover:bg-[#FAFAFA] dark:hover:bg-[#252525] transition-colors group">
                ...
                {entry.status === "ready" && entry.buffer ? (
                  <button onClick={() => downloadFile(entry)} className="opacity-0 group-hover:opacity-100 inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium text-[#6366F1] border border-[#6366F1]/30 rounded-md hover:bg-[#6366F1]/10 transition-all" aria-label={`Download ${basename(entry.name)}`}>
      ...
      {(uiState === "idle" || uiState === "error") && (
        <div className="mt-6 flex flex-wrap gap-3 justify-center text-xs text-[#A3A3A3] dark:text-[#525252]">
          <span className="flex items-center gap-1">
            Open a .rar archive?{" "}
            <a href="/tools/unrar" className="text-[#6366F1] hover:underline inline-flex items-center gap-0.5">Open RAR Online <ChevronRight size={10} /></a>
          </span>
          <span className="flex items-center gap-1">
            Create a ZIP instead?{" "}
            <a href="/tools/zip-creator" className="text-[#6366F1] hover:underline inline-flex items-center gap-0.5">ZIP Creator <ChevronRight size={10} /></a>
          </span>
        </div>
      )}
    </div>
