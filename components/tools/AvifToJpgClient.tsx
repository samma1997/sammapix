...
// Converts AVIF → JPG via Canvas (client-side, zero upload).
// Browsers decode AVIF natively via HTMLImageElement / createImageBitmap.
// AVIF can carry an alpha channel → fill background with the chosen color.
async function convertAvifToJpg(
  ...
  try {
    const img = await new Promise<HTMLImageElement>((resolve, reject) => {
      ...
      el.onerror = () => reject(new Error("Cannot decode AVIF — your browser may not support this format"));
      el.src = url;
    });
    ...
    // Draw on transparent canvas to detect alpha
    ctx.drawImage(img, 0, 0);
    const hadTransparency = detectTransparency(ctx, canvas.width, canvas.height);

    // Redraw: clear + fill bg + draw (JPG has no alpha channel)
...
export default function AvifToJpgClient() {
  ...
  const convertAll = useCallback(async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        const { blob, hadTransparency } = await convertAvifToJpg(it.file, quality, bgColor);
  ...
  const downloadAll = async () => {
    ...
    saveAs(blob, "sammapix-avif-to-jpg.zip");
  };
        ...
        {uiState === "idle" && items.length === 0 && (
          ...
          >
            ...
            <p className="text-sm font-medium text-[#171717] dark:text-[#E5E5E5] mb-1">
              Drop your AVIF files here or click to browse
            </p>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-[#1E1E1E] border border-[#E5E5E5] dark:border-[#2A2A2A] rounded-md p-4">
              <div className="flex flex-wrap items-center gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-[#525252] dark:text-[#A3A3A3] mb-1.5">
                    Background (for transparent AVIFs)
                  </label>
            ...
            <div className="space-y-1.5">
              {items.map((it) => (
                ...
                >
                  ...
                  {it.status === "error" && (
                    <span className="text-xs text-red-500 shrink-0 max-w-[140px] truncate" title={it.error}>
                      {it.error}
                    </span>
                  )}
                </div>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {uiState === "idle" && (
                ...
                >
                  Convert {items.length} AVIF {items.length === 1 ? "file" : "files"} to JPG
                </button>
              )}
          ...
          <div className="mt-4 p-4 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900 rounded-md">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-semibold text-[#171717] dark:text-[#E5E5E5]">
                Converted {doneCount} of {items.length} AVIF {items.length === 1 ? "file" : "files"}
              </span>
            </div>
            ...
            <p className="text-[11px] text-[#737373] dark:text-[#A3A3A3] mt-1.5">
              Note: JPG is typically larger than AVIF for the same quality. JPG is better for compatibility with apps and email clients that don&apos;t support AVIF yet.
            </p>
          </div>
        )}

        {/* WARNING: transparency lost → suggest PNG */}
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-[#171717] dark:text-[#E5E5E5] mb-1">
                  {lostAlpha.length === doneItems.length
                    ? "Your AVIF had transparency — now flattened on the chosen background"
                    : `${lostAlpha.length} of your AVIF${lostAlpha.length > 1 ? "s had" : " had"} transparency — flattened on ${background === "white" ? "white" : "black"} background`}
                </p>
                <p className="text-xs text-[#525252] dark:text-[#A3A3A3] mb-3 leading-relaxed">
                  JPG cannot store an alpha channel. If you need to preserve transparency (stickers, logos, graphics), convert to PNG instead.
                </p>
              </div>
            </div>
          </div>
