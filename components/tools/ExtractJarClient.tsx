...
const ACCEPT           = ".jar,.war,application/java-archive,application/x-java-archive";
...
function totalBytes(entries: JarEntry[]): number {
...
export default function ExtractJarClient() {
  ...
  const startExtraction = useCallback(async (file: File) => {
    ...
    try {
      ...
      const initial: JarEntry[] = fileEntries.map(({ file: f, path }) => ({
        name: entryName(f, path),
        size: f.size,
        status: "pending",
      }));
      setEntries([...initial]);
      trackEvent("extract_jar_extracting", { files: filteredTotal });

      const extracted: JarEntry[] = [...initial];
      ...
      setUiState("results");
      setProgress(100);
      trackEvent("extract_jar_extracted", { files: filteredTotal });
  ...
  const startListOnly = useCallback(async (file: File) => {
    ...
    try {
      ...
      const list: JarEntry[] = filesArray
        .filter(({ file: f, path }) => {
  ...
  const handleFile = useCallback((file: File) => {
    ...
    if (!lname.endsWith(".jar") && !lname.endsWith(".war")) {
      setErrorMsg("Please select a valid .jar or .war file.");
      setUiState("error");
      return;
    }
    ...
    if (file.size > limitBytes && !isPro) {
      startListOnly(file);
      trackEvent("extract_jar_gate_shown", { size_mb: Math.round(file.size / 1024 / 1024) });
      return;
    }
    ...
    pollTimerRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (data.paid) {
          ...
          trackEvent("extract_jar_daypass_unlocked_poll");
          startExtraction(file);
        }
  ...
  const handleUnlockClick = useCallback(async () => {
    ...
    trackEvent("extract_jar_daypass_checkout_start");

    try {
      const res = await fetch("/api/checkout/day-pass", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ source: "/tools/extract-jar" }),
      });
  ...
  const downloadFile = useCallback((entry: JarEntry) => {
    if (!entry.buffer) return;
    const blob = new Blob([entry.buffer]);
    saveAs(blob, basename(entry.name));
    trackEvent("extract_jar_download_single", { name: entry.name });
  }, []);
  ...
  const downloadAllAsZip = useCallback(async () => {
    ...
    if (!isPro) {
      trackEvent("extract_jar_zip_gate", { files: ready.length });
    ...
    try {
      ...
      const archiveName = archiveFile
        ? archiveFile.name.replace(/\.(jar|war)$/i, ".zip")
        : "archive.zip";
      saveAs(blob, archiveName);
      trackEvent("extract_jar_download_zip", { files: ready.length });
    } catch {
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-4 pb-16">
        ...
        <div className="mb-4 flex items-start gap-3 px-4 py-3 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/50 rounded-xl">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-700 dark:text-emerald-400 mt-0.5">
              Drop your .jar or .war file again below to extract it instantly.
            </p>
          </div>
        </div>
      )}
      ...
      {uiState === "idle" && (
        <div
          ...
          role="button"
          aria-label="Drop a .jar or .war file or click to select"
        >
          <input
            ref={fileInputRef}
            type="file"
          ...
          <p className="text-xs text-[#A3A3A3] dark:text-[#525252] mt-1">
            Java archives (.jar, .war) · Free up to 200 MB
          </p>
        </div>
      )}
      ...
      {uiState === "error" && (
        <div className="border border-red-200 dark:border-red-900/40 bg-red-50 dark:bg-red-950/20 rounded-2xl p-6 flex flex-col items-center text-center gap-3">
          ...
          <p className="text-xs text-[#737373] dark:text-[#A3A3A3]">
            Make sure the file is a valid .jar or .war archive and is not corrupted.
          </p>
          <button
            onClick={handleReset}
      ...
      {uiState === "results" && (
        <div className="space-y-4">
          ...
          {guestEmail && !session?.user?.email && (
            <div className="flex items-start gap-2 px-3 py-2.5 bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800/40 rounded-xl">
              ...
              <p className="text-xs text-indigo-800 dark:text-indigo-300">
                ...
                <a
                  href={`/auth/signin?callbackUrl=/tools/extract-jar`}
          ...
          {/* Cross-sell for class files */}
          {(() => {
            const classCount = readyEntries.filter((e) =>
              /\.(class|java|kt|xml|json|properties)$/i.test(e.name)
            ).length;
            ...
            return (
              <div className="px-4 py-3.5 bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800/40 rounded-xl">
                <p className="text-xs font-semibold text-indigo-900 dark:text-indigo-200 mb-2.5">
                  {classCount} source file{classCount !== 1 ? "s" : ""} found. Need to work with archives next?
                </p>
                <div className="flex flex-wrap gap-2">
                  <Link
                    href="/tools/zip-creator"
                    onClick={() => trackEvent("extract_jar_crosssell_click", { to: "zip-creator" })}
                  ...
                  <Link
                    href="/tools/compress"
                    onClick={() => trackEvent("extract_jar_crosssell_click", { to: "compress" })}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-white dark:bg-[#191919] text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/40 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-950/50 transition-colors"
                  >
      ...
      {(uiState === "idle" || uiState === "error") && (
        <div className="mt-6 flex flex-wrap gap-3 justify-center text-xs text-[#A3A3A3] dark:text-[#525252]">
          <span className="flex items-center gap-1">
            Need to open an APK?{" "}
            <a href="/tools/apk-extractor" className="text-[#6366F1] hover:underline inline-flex items-center gap-0.5">
      ...
      <FreeSignupAdBar tool="extract-jar" />
