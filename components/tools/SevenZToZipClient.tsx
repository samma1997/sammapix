...
export default function SevenZToZipClient() {
  ...
  const buildZip = useCallback(
    async (extractedEntries: SevenZEntry[], sourceFile: File) => {
      ...
      try {
        ...
        const name = sourceFile.name.replace(/\.7z$/i, ".zip");
        setZipBlob(blob);
        setZipName(name);
        setUiState("done");
        trackEvent("7z_to_zip_done", { files: extractedEntries.length });
  ...
  const startExtraction = useCallback(
    async (file: File, pwd?: string) => {
      ...
      try {
        ...
        setEntries([...initial]);
        trackEvent("7z_to_zip_extracting", { files: filteredTotal });
        ...
        if (
          ...
          setErrorMsg(
            msg ||
              "This 7Z could not be extracted. It may be corrupted or use an unsupported compression method."
          );
          setUiState("error");
        }
      }
  ...
  const handleFile = useCallback(
    (file: File) => {
      ...
      if (file.size > limitBytes && !isPro) {
        startListOnly(file);
        trackEvent("7z_to_zip_gate_shown", {
      ...
      pollTimerRef.current = setInterval(async () => {
        ...
        try {
          ...
          if (data.paid) {
            ...
            trackEvent("7z_to_zip_daypass_unlocked");
  ...
  const handleUnlockClick = useCallback(async () => {
    ...
    trackEvent("7z_to_zip_daypass_checkout_start");

    try {
      const res = await fetch("/api/checkout/day-pass", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ source: "/tools/7z-to-zip" }),
      });
  ...
  const handleDownload = useCallback(() => {
    ...
    trackEvent("7z_to_zip_download", { name: zipName });
  }, [zipBlob, zipName]);
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-4 pb-16">
        ...
        <div className="mb-4 flex items-start gap-3 px-4 py-3 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/50 rounded-xl">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-700 dark:text-emerald-400 mt-0.5">
              Drop your .7z file again below to convert it.
            </p>
          </div>
        </div>
      )}
      ...
      {uiState === "needs_password" && (
        <div className="border border-[#E5E5E5] dark:border-[#2A2A2A] rounded-2xl p-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <p className="text-sm font-medium text-[#171717] dark:text-[#E5E5E5]">
              This 7Z archive is password-protected
            </p>
          </div>
      ...
      {uiState === "error" && (
        <div className="border border-red-200 dark:border-red-900/40 bg-red-50 dark:bg-red-950/20 rounded-2xl p-6 flex flex-col items-center text-center gap-3">
          ...
          <p className="text-xs text-[#737373] dark:text-[#A3A3A3]">
            Multi-volume archives (.7z.001, .7z.002) are not supported. For split archives, reassemble them first with 7-Zip on desktop.
          </p>
          <button
            onClick={handleReset}
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2.5 px-4 py-3 bg-[#F0FDF4] dark:bg-[#052E16]/40 border border-[#BBF7D0] dark:border-[#166534]/40 rounded-xl">
            ...
            <p className="text-sm text-emerald-800 dark:text-emerald-300">
              ...
              from 7Z to ZIP — ready to download
            </p>
          </div>
          ...
          {guestEmail && !session?.user?.email && (
            <div className="flex items-start gap-2 px-3 py-2.5 bg-sky-50 dark:bg-sky-950/30 border border-sky-200 dark:border-sky-800/40 rounded-xl">
              ...
              <p className="text-xs text-sky-800 dark:text-sky-300">
                ...
                <a
                  href={`/auth/signin?callbackUrl=/tools/7z-to-zip`}
          ...
          <div className="flex items-center justify-between">
            ...
            <p className="text-xs text-[#A3A3A3] dark:text-[#525252]">
              Multi-volume archives (.7z.001) are not supported.
            </p>
          </div>
        </div>
      )}
      ...
      {(uiState === "idle" || uiState === "error") && (
        <div className="mt-6 flex flex-wrap gap-3 justify-center text-xs text-[#A3A3A3] dark:text-[#525252]">
          <span className="flex items-center gap-1">
            Need to open 7Z and download files individually?{" "}
            <a
              href="/tools/open-7z"
      ...
      <FreeSignupAdBar tool="7z-to-zip" />
