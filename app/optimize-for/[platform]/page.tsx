...
import {
  getAllOptimizePlatforms,
  getOptimizePlatform,
  getOptimizePlatformCanonical,
  getOptimizeAspectRatio,
} from "@/lib/optimize-platforms";
import { APP_URL } from "@/lib/constants";

// ─── Static params ─────────────────────────────────────────────────────────

export function generateStaticParams() {
  return getAllOptimizePlatforms().map((p) => ({ platform: p.slug }));
}
...
export async function generateMetadata({
  ...
  const title = platform.seoTitle ?? `Optimize Images for ${platform.displayName} Free Online`;
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `SammaPix - Optimize Images for ${platform.displayName}`,
        },
      ],
    },
    twitter: {
...
function buildJsonLd(slug: string) {
  ...
  const softwareApp = {
    ...
    "@id": `${canonical}#app`,
    name: `SammaPix - Optimize for ${platform.displayName}`,
    description: platform.metaDescription,
    url: canonical,
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      },
      {
        "@type": "ListItem",
        position: 3,
        name: `Optimize for ${platform.displayName}`,
        item: canonical,
      },
    ],
  };
...
export default async function OptimizePlatformPage({
  params,
}: {
  params: Promise<{ platform: string }>;
}) {
  ...
  const relatedPlatforms = platform.related
    .map((r) => getOptimizePlatform(r))
    .filter(Boolean) as NonNullable<ReturnType<typeof getOptimizePlatform>>[];
        ...
        <div className="max-w-3xl mx-auto">
          <ol className="flex items-center gap-1.5 text-xs text-[#A3A3A3]">
            ...
            <li className="text-[#525252]">
              Optimize for {platform.displayName}
            </li>
          </ol>
        </div>
      </nav>
      ...
      <section className="px-4 sm:px-6 pt-10 pb-12">
        <div className="max-w-3xl mx-auto">
          <h1 className="text-2xl font-bold text-[#171717] dark:text-[#E5E5E5] mb-4">
            Optimize Images for {platform.displayName} Free Online
          </h1>
      ...
      <section className="px-4 sm:px-6 py-12 border-t border-[#E5E5E5] dark:border-[#2A2A2A]">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-lg font-semibold text-[#171717] dark:text-[#E5E5E5] mt-0 mb-4">
            {platform.displayName} Recommended Image Sizes
          </h2>
          <p className="text-sm text-[#737373] leading-relaxed mb-6">
            All recommended {platform.displayName} image dimensions for 2026.
            Upload at these exact sizes and compress before uploading to avoid
            any quality loss from platform-side re-compression.
          </p>
                ...
                {platform.sizes.map((size, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-[#525252] dark:text-[#A3A3A3] border-b border-[#E5E5E5] dark:border-[#2A2A2A] last:border-0">
                      {getOptimizeAspectRatio(size.width, size.height)}
                    </td>
      ...
      <section className="px-4 sm:px-6 py-12 border-t border-[#E5E5E5] dark:border-[#2A2A2A]">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-lg font-semibold text-[#171717] dark:text-[#E5E5E5] mt-10 mb-4">
            How to Optimize Images for {platform.displayName}
          </h2>
        ...
        <div className="max-w-3xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#737373] leading-relaxed">
              ...
              to reduce file size without quality loss, or{" "}
              <Link
                href="/tools/webp"
                className="underline underline-offset-2 hover:text-[#171717] dark:hover:text-[#E5E5E5] transition-colors"
              >
              ...
              for an extra 25-35% size reduction.
            </p>
          </div>
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {relatedPlatforms.map((related) => (
                <Link
                  key={related.slug}
                  href={`/optimize-for/${related.slug}`}
                ...
                >
                  <p className="text-sm font-medium text-[#171717] dark:text-[#E5E5E5] group-hover:text-[#171717] dark:group-hover:text-white mb-1">
                    Optimize for {related.displayName}
                  </p>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              {
                title: "No uploads",
                desc: "All compression happens entirely in your browser. Your images never leave your device.",
              },
              {
                title: "Batch process",
                desc: "Optimize multiple images at once with the same settings. Download all as ZIP in seconds.",
              },
              {
                title: "Free forever",
                desc: "Core optimization tools are completely free. No watermarks, no ads, no file limits.",
              },
            ].map((item) => (
              <div
                key={item.title}
      ...
      <section className="px-4 sm:px-6 py-12 border-t border-[#E5E5E5] dark:border-[#2A2A2A]">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              All optimizations <ArrowRight className="h-3.5 w-3.5" strokeWidth={1.5} />
            </Link>
            <Link
              href="/tools/compress"
