...
export async function generateMetadata({
  ...
  const title = `${target.titleKeyword} Online: Free, No Upload, Exact Size`;
...
export default async function CompressToSizePage({
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-[#191919]">
      ...
      {/* ── All target sizes (full sibling grid, internal linking) ───────── */}
      <section className="px-4 sm:px-6 py-12 border-t border-[#E5E5E5] dark:border-[#2A2A2A]">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex flex-wrap gap-2">
            {getAllTargets()
              ...
              .map((t) => (
                <Link
                  key={t.slug}
                  href={`/compress-to/${t.slug}`}
                  className="inline-flex items-center px-3 py-1.5 text-sm border border-[#E5E5E5] dark:border-[#2A2A2A] rounded-md text-[#525252] hover:border-[#A3A3A3] hover:text-[#171717] dark:text-[#E5E5E5] bg-white dark:bg-[#1E1E1E] transition-colors uppercase"
                >
                  {t.slug}
                </Link>
              ))}
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm border border-[#E5E5E5] dark:border-[#2A2A2A] rounded-md text-[#525252] hover:border-[#A3A3A3] hover:text-[#171717] dark:text-[#E5E5E5] bg-white dark:bg-[#1E1E1E] transition-colors"
            >
              Guide: Compress Without Quality Loss
              <ArrowRight className="h-3.5 w-3.5" strokeWidth={1.5} />
            </Link>
          </div>
        </div>
