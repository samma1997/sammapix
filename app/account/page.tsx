  ...
  return (
    <div className="py-16 px-4 sm:px-6">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-[#171717] tracking-tight">Account</h1>
          <p className="text-sm text-[#737373] mt-1">Manage your profile and subscription.</p>
        </div>

        {/* Profile card */}
        <div className="border border-[#E5E5E5] rounded-md p-5 bg-white">
          <div className="flex items-center gap-4">
            {user?.image ? (
              ...
              <img
                src={user.image}
                alt={user.name ?? ""}
                className="h-12 w-12 rounded-full border border-[#E5E5E5]"
              />
            ) : (
              <div className="h-12 w-12 rounded-full bg-[#F5F5F5] border border-[#E5E5E5] flex items-center justify-center">
        ...
        <div className="border border-[#E5E5E5] rounded-md p-5 bg-white">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-semibold text-[#171717]">Subscription</h2>
          </div>

          {isPro ? (
            <>
              <div className="flex items-center gap-3 p-3 bg-[#FAFAFA] border border-[#E5E5E5] rounded-md mb-4">
                <div className="h-8 w-8 rounded-full bg-[#171717] flex items-center justify-center shrink-0">
                  <Crown className="h-3.5 w-3.5 text-white" />
                </div>
                <div>
                  <p className="text-sm font-medium text-[#171717]">Pro plan — active</p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-4">
                {PRO_TOOLS.map((t) => (
                  <div key={t.name} className="flex items-center gap-2 p-2 bg-[#FAFAFA] border border-[#E5E5E5] rounded text-xs">
                    <span className="text-[#525252]">{t.icon}</span>
                    <span className="font-medium text-[#171717]">{t.name}</span>
              ...
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-2 px-4 py-2 border border-[#E5E5E5] rounded-md text-sm font-medium text-[#525252] bg-white hover:bg-[#F5F5F5] transition-colors disabled:opacity-60"
              >
                {portalLoading ? (
            ...
            <>
              <div className="flex items-center gap-3 p-3 bg-[#FAFAFA] border border-[#E5E5E5] rounded-md mb-4">
                <div>
                  <p className="text-sm font-medium text-[#171717]">Free plan</p>
